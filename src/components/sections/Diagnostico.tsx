...
const DiagnosticoForm = dynamic(
  ...
  { loading: () => <p role="status" className="py-5 text-sm text-ink-3">Cargando el asistente…</p> },
);
...
export function Diagnostico() {
  ...
  return (
    <section id="diagnostico" aria-label={diagnostico.kicker} className="wrap pb-12 768:pb-16">
      <details className="group border-y border-line" onToggle={(event) => { if (event.currentTarget.open) setLoaded(true); }}>
        <summary className="flex cursor-pointer list-none items-center justify-between gap-6 py-6 marker:content-none [&::-webkit-details-marker]:hidden">
          <span>
            <span className="font-mono text-[10px] uppercase tracking-[0.14em] text-ink-3">{diagnostico.kicker}</span>
            <span className="mt-2 block text-lg font-semibold">{diagnostico.resumen}</span>
          </span>
          <span aria-hidden className="flex h-10 w-10 shrink-0 items-center justify-center border border-line font-mono text-xl text-cobalt group-open:bg-cobalt group-open:text-white">
        ...
        <div className="border-t border-line pb-8 pt-7">
          ...
          <p className="mb-7 mt-3 max-w-[700px] text-base leading-relaxed text-ink-2">{diagnostico.apoyo}</p>
